...
def execute_trading_strategy(all_results, daily_signals, threshold, strategy):
    """
    Execute the rebalancing trading strategy
    """
    ...
    for i, signal in enumerate(valid_signals):
        ...
        # Progress update
        if (i + 1) % 10 == 0:
            print(f"  Processed {i + 1}/{len(valid_signals)} signals...")
    
    # Final portfolio valuation if we still have positions
    if strategy.current_portfolio is not None:
        final_date = valid_signals[-1]['date'] if valid_signals else daily_signals[-1]['date']
...
def enter_new_position(strategy, all_results, entry_date, assets, signal_strength):
    """
    Enter new position with equal allocation across assets
    """
...
def track_portfolio_value(strategy, all_results, current_date):
    """
    Track portfolio value without rebalancing
    """
    if strategy.current_portfolio is None:
        return
    ...
    for asset, position in strategy.current_portfolio.items():
        ...
        if current_price is not None:
            ...
            # If we can't get current price, use entry value
    ...
    strategy.portfolio_history.append({
        ...
        'unrealized_pnl': total_value - strategy.current_capital,
        'unrealized_pnl_pct': (total_value / strategy.current_capital - 1) * 100,
        'days_held': (current_date - strategy.current_entry_date).days,
        'assets': list(strategy.current_portfolio.keys())
    })
...
def create_strategy_visualizations(strategy, analysis, output_dir):
    """
    Create comprehensive strategy performance visualizations
    """
    ...
    entries = portfolio_df[portfolio_df['action'] == 'ENTER']
    exits = portfolio_df[portfolio_df['action'].isin(['REBALANCE', 'FINAL EXIT'])]
    
    ax1.scatter(entries['date'], entries['portfolio_value'], 
               color='green', s=100, marker='^', label='Entry Points', zorder=5)
    ax1.scatter(exits['date'], exits['portfolio_value'], 
               color='red', s=100, marker='v', label='Exit Points', zorder=5)
    ...
    ax1.set_title('Portfolio Value Over Time', fontweight='bold', fontsize=16)
    ...
    if strategy.trade_history:
        ...
        ax2.hist(trades_df['trade_pnl_pct'], bins=15, alpha=0.7, 
                color='steelblue', edgecolor='black')
        ax2.axvline(trades_df['trade_pnl_pct'].mean(), color='red', 
    ...
    if strategy.trade_history:
        ...
        ax3.plot(trades_df['exit_date'], trades_df['trade_pnl_pct'], 
                marker='o', linewidth=2, markersize=6, color='purple')
        ax3.axhline(0, color='black', linestyle='-', alpha=0.5)
        ax3.axhline(trades_df['trade_pnl_pct'].mean(), color='red', 
                   linestyle='--', alpha=0.7, label=f'Average: {trades_df["trade_pnl_pct"].mean():.1f}%')
        
        ax3.set_title('Trade Returns Over Time', fontweight='bold')
        ax3.set_xlabel('Exit Date')
